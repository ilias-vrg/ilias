...
class UNICModel(GenericFeatureExtractor):
    def __init__(self, **kargs):
        super(UNICModel, self).__init__(**kargs)
        assert self.pooling == "head", NotImplementedError(
            "Other pooling operations are not implemented for UNIC"
        )

        self.model, _ = load_student_encoder_from_checkpoint(
            f"feature_extraction/pretrained_models/unic/{self.name}.pth"
        )
        self.pad = MultiplierPadding(14)

    def _extract_features(self, x):
        x = self.pad(x)
        x = self.model(x)["x_norm_clstoken"]

        if self.pooling == "head":
            return x
        raise NotImplementedError("Local pooling not implemented for UNIC!")
